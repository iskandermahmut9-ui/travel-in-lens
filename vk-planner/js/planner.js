  ...
  async function saveAsJPG() {
        ...
        try {
            ...
            finalCanvas.toBlob(async (blob) => {
                ...
                closeBtn.style.cssText = 'padding:14px; background:#444; color:#fff; border:none; border-radius:8px; font-weight:800; font-size:13px; cursor:pointer; flex:1;';
                closeBtn.onclick = () => document.body.removeChild(overlay);
                
                const downloadBtn = document.createElement('button');
                downloadBtn.innerHTML = "📥 СКАЧАТЬ";
                downloadBtn.style.cssText = 'padding:14px; background:#FF5722; color:#fff; border:none; border-radius:8px; font-weight:800; font-size:13px; cursor:pointer; flex:1;';
                downloadBtn.onclick = () => {
                    const link = document.createElement('a'); link.download = "travel-in-lens.ru.jpg"; link.href = url;
                    document.body.appendChild(link); link.click(); document.body.removeChild(link);
                };
