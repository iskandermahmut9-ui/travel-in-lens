  ...
  // --- 0. ИНИЦИАЛИЗАЦИЯ И БЕСШОВНАЯ АВТОРИЗАЦИЯ VK ---
    try {
        if (window.vkBridge) {
            ...
            // --- ЗАЩИЩЕННАЯ МАГИЯ БЕСШОВНОГО ВХОДА ---
            setTimeout(async () => {
                ...
                try {
                    ...
                    if (vkUser && vkUser.id && supabase) {
                        ...
                        // Достаем подпись (sign) из ссылки, по которой ВК открыл приложение
                        const urlParams = new URLSearchParams(window.location.search);
                        const vkSign = urlParams.get('sign');

                        const vkEmail = `vk_${vkUser.id}@travel-in-lens.ru`;
                        // Используем сложную подпись ВК как пароль (если она есть)
                        const vkPass = vkSign ? `sign_${vkSign}` : `vk_secure_pass_${vkUser.id}!`;

                        let { data, error } = await supabase.auth.signInWithPassword({ email: vkEmail, password: vkPass });
                        
                        if (error) {
                            btn.innerText = 'БАЗА: РЕГИСТРАЦИЯ...'; 
                            ...
                            if (!regError && regData.user) {
                                ...
                                showToast("Ошибка БД: " + regError.message);
